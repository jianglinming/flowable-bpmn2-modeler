...
public class FlowableCookieFilter extends OncePerRequestFilter {
    ...
    protected boolean validateRequiredPriviliges(HttpServletRequest request, HttpServletResponse response, FlowableAppUser user) {
        ...
        // rest calls handled by Spring Security conf
        String pathInfo = request.getPathInfo();
        if (isRootPath(request) || !pathInfo.startsWith("/rest")) {
            if (requiredPrivileges != null && requiredPrivileges.size() > 0) {
                if (user.getAuthorities() == null || user.getAuthorities().size() == 0) {
                    ...
                    return matchingPrivileges == requiredPrivileges.size();
                }
            }
        }
    ...
    protected void redirectToLogin(HttpServletRequest request, HttpServletResponse response, String userId) {
        try {
            ...
            String baseRedirectUrl = idmAppUrl + "?redirectUrl=";
            if (redirectUrlOnAuthSuccess != null) {
                response.sendRedirect(baseRedirectUrl + URLEncoder.encode(URLEncoder.encode(redirectUrlOnAuthSuccess, "UTF-8"), "UTF-8"));
            } else {
                response.sendRedirect(baseRedirectUrl + URLEncoder.encode(URLEncoder.encode(request.getRequestURL().toString(), "UTF-8"), "UTF-8"));
            }
